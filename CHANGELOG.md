...
Additive and number-preserving with one exception, and the exception is the first thing to read. Nothing here changes a figure any existing report publishes — the new record fields default to null, and the new subcommand only reads transcripts and writes files you name — but the synthetic-corpus fix under **Fixed** changes what a synthetic `ascep bench` run *sends*, so re-running an existing bench config against the same engine will not reproduce the numbers it produced before. It should not: the old numbers described a prompt roughly eight times longer than the config declared.
