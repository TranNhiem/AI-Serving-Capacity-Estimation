...
- **A hero figure at the top of the README, drawn by `tools/make_figure.py` rather than by hand.** `assets/ascep-capacity-model.svg` shows the whole protocol in one frame: five declarations feed four independent capacity floors, capacity is the minimum of the four, the floor that binds is named, and the same cluster is then reported at four tiers with a provenance tag on each. It is generated because a drawn figure drifts — someone edits the protocol and the picture keeps teaching the old rule — so `tests/test_figure.py` regenerates it and compares bytes, the same discipline `test_report_conformance.py` applies to the published examples. The numbers in it are labelled illustrative and are not drawn from any campaign: the four published GB200 bundles are awaiting re-measurement with the fixed harness and two have withdrawn top rungs, and a figure that carries a number the repository later retracts would be the failure ASCEP exists to prevent, printed at the top of the page. The remaining tests cover what GitHub actually renders — no `<script>`, no `<foreignObject>`, no remote fetches, and a document that parses, after a first draft whose font-family string closed an XML attribute early and rendered as a browser error page rather than as a slightly wrong drawing.
...
- **Chapter 7 documents the seven optional `workload` config keys it has been citing for three releases without describing.** `media_root`, `image_input_transport`, `media_url_prefix`, `media_max_records`, `prompt_field`, `strip_media_placeholders` and `replay_sessions` are all accepted by `ascep bench`, and the loader's refusal messages cite "section 9" and "section 10" for them — sections that named none of them. An operator hitting one of those refusals was sent to a chapter that could not tell them what the key was for, which is a worse failure than an undocumented key, because it looks like the documentation is wrong rather than absent. They are a second table below the required one, with their defaults and the configurations each is refused alongside, and the note above it says why they are separate: making a new capability a required key would invalidate every published bench config overnight, including configs whose runs are already cited by a report. The `corpus` row in the required table is corrected at the same time — it said the prompt is read from the record's `messages` field, which stopped being true when `prompt_field` was made reachable and defaulted to `conversations`.
...
- **The 2,251-line `tests/test_cli_bench.py` is three modules over one support module, and the coupling that made it unsplittable is gone.** The command has one contract and several subjects, and the file had grown to hold all of them: `tests/test_cli_bench.py` now keeps the contract (refuse before the first request, write beside the config, never grade yourself), `tests/test_cli_bench_workload.py` keeps how a declaration becomes traffic (the optional media keys, the corpus keys, the three `output_tokens`/`ignore_eos` states, the session replay), and `tests/test_cli_bench_draft.py` keeps what the draft report claims (which floor was observed, which rows are tagged (U), and the rule that the Measured tier is the engine ceiling while the Sustainable tier is not). The complete config every test subtracts from, the four declaration documents and the offline stand-in for a server live in `tests/bench_cli_support.py`, which holds no assertion at all — a helper module that grew a test would be a test nobody runs by name. One test blocked the split and is the reason it is worth recording: the session-replay case reused the text run's schema check by rebinding `_config` in the module globals and calling that test function directly. The instinct was right — a second, weaker copy of a validation check is how two checks drift apart without either failing — but a test that patches another test's module state cannot move away from it, which is what a 2,251-line file is made of. Both now call one shared assertion. Same 116 tests, collected before and after to confirm none were lost in the move.

### Removed

- **`tools/migrate_v02_multimodal.py`, the one-shot v0.1 → v0.2 backfill, three releases after the release it migrated to.** Every document in the tree has been through it, it is idempotent so a second run does nothing, it has no test, and nothing in CI or the documentation runs it. Its own docstring gave the reason to keep it — the change stays auditable — and git is where that audit lives: the script and every document it edited are in the same commit, which is a better record than a file that no longer matches the schemas it was written against. What keeping it cost is the thing `tools/` is for. That directory is where a contributor looks for what they are supposed to run, and one of its three entries was an untested script that walks every JSON document in the repository and rewrites it.

## [0.7.0] — 2026-09-13

**Nothing in this release moves a number, and every published example recomputes bit-identically; what breaks is validation, on purpose.** ASCEP's serving schema was written against engines that batch — vLLM, SGLang, continuous batching, `max_num_seqs`, a container digest — and it now has to describe llama.cpp, Ollama, LM Studio and MLX on the DGX Spark and Apple Silicon, whose differences change the capacity arithmetic rather than merely the vocabulary. Seven serving properties become required at once, which the versioning rule makes a conformance-gate change, and under 0.x the minor is the breaking axis: a 0.6 `serving.json` that says nothing untrue now needs seven lines added, and every figure it fed stays exactly as published. What the gate stops is the arithmetic failure they share: on a serial or slotted runtime, ASCEP's throughput floor divides aggregate tok/s by per-user demand and multiples by admitted concurrency, and a machine that admits one request produces from that division a concurrency claim it physically cannot honour — in a report that looks conforming.

### Added

- **`runtime_class` is the structural fact about admitted concurrency, and it takes no null and no (U) reason for the same reason `memory_architecture` does not.** The enum is `server-batched` | `slotted` | `serial` — one forward pass scheduling many sequences; a fixed number of independent slots, each with its own context; or one request at a time, with the second queued or refused. Every conditional in this release keys off it, and a null discriminator leaves the document in no branch at all. It is deliberately not a synonym for `batching_mode`: on a runtime that admits one request there is no scheduler to describe, so letting a serial report wear a batching label would certify a mechanism that does not exist. Every reporter knows whether their server accepts a second request while the first is generating.

- **`parallel_slots` and `context_tokens_per_slot` (+ their `_u_reason` siblings) are the two numbers the undivided reading gets wrong by exactly a factor of slots.** The first is how many requests the runtime admits before queueing — the number the throughput floor is allowed to multiply by. Neither is unconditionally required: a server-batched engine has no slots, and requiring both there would put a null and a (U) sentence on every vLLM report to say "not applicable", which is the erosion that makes the real (U) tags stop being read. The conditionals demand them where they exist. The second is the per-session window after the configured context is divided among slots: `llama-server -c 32768 --parallel 4` gives each slot 8192 tokens, and quoting the undivided window lets a reader size KV for four concurrent 32k sessions, over-promising by 4x with nothing in the report to tell them otherwise.

- **`context_overflow_policy` names what happens when a request exceeds its window, because two of the three answers corrupt the measurement silently.** `refuse` is the only policy that leaves a declared workload honest. Under `truncate`, and under llama.cpp's `shift` — which silently evicts the oldest tokens and keeps generating — a sweep whose prompts outgrow the window is measuring a shorter workload than the one declared, and the throughput it reports is high for exactly that reason, with the elevation reading as good news rather than as a units error.

- **`kv_cache_type_k` and `kv_cache_type_v` (+ their `_u_reason` siblings) replace a boolean answer to a numeric question.** `kv_cache_quantized: on|off` says that quantisation happened without saying how many bytes it left, and the KV floor needs bytes per element, so it cannot be computed from the boolean at all. K and V are separate fields because llama.cpp quantises them separately (`-ctk q8_0 -ctv f16`) and asymmetric settings are common; a single type would average two element sizes that differ by a factor of two into one figure that fits neither.

- **`weights_residency` distinguishes pinned weights from a measurement of the SSD.** `resident` means the weights are in memory; `mmap-paged` means they are mapped from disk and evictable. The weights floor assumes resident, and on a unified machine under memory pressure an mmap-paged model pages to NVMe mid-decode, after which the measured tok/s is characterising the storage, not the accelerator. Declaring `mmap-paged` marks the weights floor as a soft bound rather than a hard one; `unknown` is a legal honest value, because a required enum whose honest values are hard to establish teaches reporters to pick a flattering one, after which the field certifies nothing.

- **`runtime_build` and `model_artifact_sha256` (+ their `_u_reason` siblings) answer the rule that a report may not pin nothing.** A local report can legitimately have no container digest, which previously left it free to identify neither the code nor the bytes: `runtime_build` carries the build identity where the version string does not (llama.cpp's `b4589 (f7f1d9a)` is an identity; "llama.cpp" plus a date is not), and `model_artifact_sha256` carries the digest of the weights file the server actually loaded. Quantisation names are not identities — two files both called `Q4_K_M` from two requantisers differ in which tensors were left at higher precision, and therefore in both quality and size — so the digest is the only thing that makes a local run reproducible. The digest is deliberately *not* required anywhere: hashing a 40 GB file is a real cost, a report that declines it with a reason is still auditable, and a required digest would be widely faked.

- **This is the first conditional logic in `serving.schema.json`, appended as an `allOf` of four branches in the order listed below, and it exists so that the field a floor multiplies by cannot be a shrug.** A `serial` runtime must declare `parallel_slots` as the literal `1` — its admitted concurrency is 1, writing the 1 is a statement where a null is a shrug, and this is the number the throughput floor multiplies by — and must declare `batching_mode` as null, since a runtime that admits one request has no scheduler; the existing null-justification check then demands the (U) sibling on its own, with no second mechanism added in the schema. A `slotted` runtime must declare `parallel_slots` and `context_tokens_per_slot` as non-null integers. Declaring `kv_cache_quantized: "on"` without naming both element types leaves the KV floor uncomputable — worse than declaring `off`, which at least implies a known element size — so `on` now requires both. A fifth branch was drafted and cut: it would have made a null `container_digest` require a non-null `runtime_build`, and three published examples in this repository declare both that and `framework_version` null, with a (U) reason that says in as many words that the report is thereby partial. The grade already carries it. A validator rejection would instead teach reporters to invent a build string, which is the failure the rule was written to prevent, so it lives in R11 as a grading rule.

- **`protocol/03-serving.md` gains a new final section, "Runtimes that do not batch", with four rules appended after the chapter's highest existing number rather than renumbering any rule a published report may cite.** The first requires declaring `runtime_class` and forbids publishing a capacity claim above one concurrent user on a `serial` runtime, because there is no measurement behind such a claim, only arithmetic. The second sets the per-slot window, not `max_model_len`, as the bound on one user's KV and the input to the KV floor. The third requires a non-`refuse` overflow policy to be stated together with the longest prompt in the sweep, and requires the report to say when any prompt in the ladder exceeded the window — that run measured a different workload than the one declared. The fourth requires a report to pin its serving code by at least one of three — a `container_digest`, a `framework_version` naming a released build, or a `runtime_build` — and holds a report that pins none of them to partial, saying so in the justification rather than leaving reproduction to look possible.

### Changed

- **Four field descriptions widened and none was renamed, because renaming to fix a sentence would break every existing report and tool.** `kv_cache_quantized` now states that `on|off` records whether quantisation happened, not what it left: the element sizes live in `kv_cache_type_k` and `kv_cache_type_v`, and quoting `on` without them leaves the KV floor uncomputable, which is stated in the field rather than left to look like a complete declaration. `batching_mode` now states that it describes the scheduler inside a server-batched runtime and that a runtime admitting one request has no scheduler to describe; the failure this prevents is a serial report carrying a batching label that validates clean and then reads as a claim about concurrency the machine cannot make.

- **`max_model_len` and `container_digest` gained one honest sentence each, aimed at the two readings this release exists to stop.** `max_model_len` now states that on a slotted runtime it is not the window that bounds one user's KV — `context_tokens_per_slot` is, and reaching for the configured figure instead over-promises by exactly the slot count. `container_digest` now states that null is acceptable on a device with no container, but that the report must then identify its code in `runtime_build` instead; the failure this prevents is a local declaration that pins neither the engine nor the weights and therefore cannot be reproduced by anyone, including its own author on another bench day.

- **`ascep init` gained a second patterned field, and the test that guarded the first was asking the wrong question.** `test_ascep_version_is_still_the_only_patterned_string` demanded that every `pattern` in the schemas have a known value in `init._KNOWN`, on the reasoning that `init` would otherwise emit its visible `TODO` placeholder and produce a document failing its own validation with `'TODO' does not match ...` — an error naming no task. `model_artifact_sha256` is patterned and deliberately nullable, so `init` emits a null and the `(U)` sibling beside it, which validates and reads as the task it is. The test now asks whether each patterned field permits a known value **or** a null, which is the property that actually decides it, and additionally refuses a nullable patterned field whose schema defines no `_u_reason` sibling — that combination would have `init` emit a bare null for C1 to reject with nowhere to write the answer. A third patterned field that forbids null still fails.

### Migration

A 0.6 serving declaration needs exactly seven lines, and on an existing vLLM report five of them are statements rather than refusals: `runtime_class` is `server-batched`, `context_overflow_policy` is `refuse` because the engine returns 400 on a request longer than `max_model_len`, `weights_residency` is `resident`, and `kv_cache_type_k` and `kv_cache_type_v` are the literal `n-a`. That last literal is load-bearing and is the one convention this release adds to the vocabulary: `n-a` means the runtime applies no per-tensor override and the model declaration's `kv_precision` governs, where null means the element type is genuinely unknown. Only one of those two states leaves the KV floor computable, and spelling the first as a null would have put a (U) tag meaning "not applicable" on thirteen declarations at once. The literal is already the house convention — `image_input_transport` carries it, and 0.6.0 sanctioned it for `driver_version` and `interconnect_intra_node`. The remaining two lines are honest refusals: `model_artifact_sha256` is null everywhere, because these checkpoints were served from a shared parallel filesystem by path and never digested, and `runtime_build` is the engine's git hash where the version string carries one and null where it does not. Thirteen declarations were migrated this way — six standalone `serving.json`, seven `serving` blocks inside a `report.json` — plus the negative corpus, which is regenerated from `examples/negative/baseline.json`. No field moved and no number changed, and each declaration keeps the `ascep_version` it was produced under, since this repository does not restamp old reports.

**The declarations inside manifest-pinned reproduction bundles were deliberately not migrated, for the same reason this repository gave when it faced the identical decision on hardware.** A bundle's every artifact is pinned by sha256 in its `manifest.json` as a record of what was declared at run time, not as a living document, and `bundle/declarations/serving.json` is among those captured bytes. Migrating it would mean rewriting evidence after the fact and then re-signing the manifest over the rewrite — exactly the sequence the manifest exists to make detectable — and a mechanism whose first sanctioned exception arrives for an innocent reason stops being a mechanism. The one sanctioned way to change bundle bytes is `tools/redact_bundle.py`, which records the original digest beside the new one, and migration is not redaction. So the bundles keep declarations the 0.7.0 schema would reject, and that is the evidence working as designed: the published `report.json` beside each bundle is the document the current schema grades, and the bundle is the record it was built from.

## [0.6.0] — 2026-09-13

**Nothing in this release moves a number, and every published example recomputes bit-identically; what breaks is validation, on purpose.** ASCEP was written against datacentre GPUs — H100, GB200: discrete HBM, tensor parallel, NVLink — and it now has to describe machines that share none of those properties: the NVIDIA DGX Spark (GB10 Grace Blackwell, 128 GB of LPDDR5X coherent unified memory at 273 GB/s, 1 PFLOP of FP4) and Apple Silicon M1–M5 Max/Ultra (36–512 GB unified, roughly 400–819 GB/s, Metal and MLX, with no published GPU FLOP/s figure at any precision), serving 10B–50B dense and MoE models. The versioning rule makes a newly required schema field a conformance-gate change, but the gate this release stiffens fails documents, not figures: a 0.5 `hardware.json` that says nothing untrue now needs one line added, and every figure it fed stays exactly as published.

### Added

- **`memory_architecture` is the discriminator a unified machine could previously only describe by pretending to be something else, and the wrong pretence prices it.** The enum is `discrete` | `unified`, always required, and every conditional introduced this cycle keys off it. Without it, a unified part arrives at a schema written for a card with its own HBM and has exactly two bad moves: declare its total system memory as the bindable budget — which overstates it by tens of GB in the **over-promising direction**, so KV floors computed against it promise concurrency no configuration of the machine can reach, and the reader who re-runs the bench misses the headline number and goes looking for the bug in their own setup — or invent a VRAM partition it does not have. The schema now refuses both.

- **The schema now refuses in both directions, not merely documents a MUST.** A `unified` declaration carrying a non-null `vram_bytes_per_gpu` is rejected, and a `discrete` one carrying a non-null `usable_memory_bytes` is rejected. The reason for enforcing rather than annotating is the reader the MUST was protecting in the first place: an unenforced MUST is worse than no rule, because a fabricated VRAM partition validates clean and then feeds straight into `kv_pool_bytes`, where it stops looking like a placeholder and starts behaving like a measurement.

- **`usable_memory_bytes` (+ `_u_reason`) is the number the OS will not let you round up.** Required when unified, it is the bytes the framework can actually bind for weights plus KV after the OS reservation, the display compositor, and the platform's wired-memory limit — on macOS `iogpu.wired_limit_mb`, ~65–75% of RAM by default — and it is deliberately not total system memory, because total system memory is the figure a report reaches for when the honest one requires running something. Beside it, `memory_type` (+ `_u_reason`) — `LPDDR5X` versus `HBM3e` — annotates the bandwidth figure so a shared DRAM bus is not read as a private GPU resource, and `thermal_behavior` (+ `_u_reason`) takes `sustained` | `throttles-under-sustained-load` | `not-assessed`. `not-assessed` is a legal honest value, and it had to be: a required enum whose honest values are hard to establish teaches reporters to pick a flattering one, after which the field certifies nothing.

- **This is the first conditional logic in `hardware.schema.json`, and it exists so that single-device parts stop having to fabricate topology.** Draft-2020-12 `allOf` with `if`/`then` gates what unconditional requirement could not: `vram_bytes_per_gpu` on `discrete` only; `usable_memory_bytes`, `memory_type` and `thermal_behavior` on `unified` only; `interconnect_intra_node` only when `gpus_per_node > 1`, with single-device parts declaring the literal `n-a` instead of inventing an interconnect between one device and itself; `interconnect_inter_node` only when `nodes > 1`; and `dense_flops_precision` whenever — and only whenever — a non-null FLOP/s figure is quoted. Fields that are required of machines that cannot have them get filled, not measured, and a schema that accepts the filling is worse than one that asks.

- **`dense_flops_precision` (+ `_u_reason`) attaches a unit to a number that was previously free to be read in the wrong one.** The enum is bf16/fp16/fp8/nvfp4/tf32/fp32 and it is required whenever `dense_bf16_flops_per_s` is non-null, because the precision the vendor's sheet means determines the prefill roofline the reader divides into — the mechanism, and the cost of getting it quietly, is the HW-11 entry below.

- **`node_exclusivity` gains `exclusive-process`, because the only honest alternative on a Mac was a banner worn forever.** The value means no other compute or graphics tenant, with the OS compositor keeping its unavoidable share. No Mac can honestly claim `exclusive` — WindowServer always holds GPU time — so absent this tier every Apple report would carry the HW-7 contamination banner permanently, and a banner that fires on an entire platform stops meaning anything long before it stops being printed.

- **`protocol/01-hardware.md` gains §1.8, "Unified memory: the wrong question and the right one", and rules HW-9 through HW-12, appended after HW-8 rather than renumbering a rule a published report may already cite.** HW-9 requires declaring `memory_architecture` and putting the method for establishing `usable_memory_bytes` in `notes`, because the figure is only auditable if its subtraction is visible. HW-10 states that on a unified part `system_ram_bytes` aliases the same silicon as the accelerator pool and MUST NOT be added to it — the only arithmetic available in that direction is double-counting. HW-11 requires a quoted FLOP/s figure to carry its precision: the Spark's 1 PFLOP is FP4, and read as dense BF16 it prices the prefill roofline about 4× too high, so an honest 60%-of-roofline measurement reads as 15% and sends the reader hunting an efficiency bug that is a units error. HW-12 requires a part that throttles to state its thermal state at the start of the window, because a cold-started sweep and a heat-soaked sweep of the same part are different measurements wearing the same model name.

### Changed

- **Three field descriptions widened and none was renamed, because renaming to fix a sentence would break every existing report and tool.** `hbm_bandwidth_bytes_s` now reads as accelerator-visible DRAM bandwidth, annotated by `memory_type`, and notes that on a unified part the bus is shared with the CPU and the display — so a CPU-bound run depresses the memory roofline itself, and a reader who did not know that would grade a legitimate measurement as a defect. `dense_bf16_flops_per_s` now reads as dense FLOP/s at whatever precision `dense_flops_precision` names; Apple publishes none, so null + (U) is the expected honest value there, and the consequence — a null prefill theoretical tier and a null `roofline_efficiency` — is now stated in the field rather than left to look like silent non-conformance. `driver_version` now sanctions the literal `n-a` where there is no separately versioned accelerator driver, since Metal ships inside the macOS release and the OS and runtime versions belong in `compute_runtime_version`; the failure this prevents is a reporter inventing a version number for a driver that does not exist, which teaches the schema that fields are things to fill rather than statements to make.

- **`ascep init` no longer names `vram_bytes_per_gpu` in the skeleton it writes, and that is the conditional mechanism working rather than a field going missing.** The skeleton is generated by walking the schema's unconditional requirements; `init.py` has always ignored `if`/`then` branches on purpose, because a skeleton that tried to satisfy every branch of a disjunction would be a document no schema accepts. So the emitted hardware block now carries `"memory_architecture": "TODO"` and nothing from either branch. The path through is the one that module's docstring already prescribes and which is now exercised for a second layer: `ascep validate` answers `'TODO' is not one of ['discrete', 'unified']`, and the moment the discriminator is filled in the next error is `'vram_bytes_per_gpu' is a required property`. A contributor following the errors reaches the same document; a contributor skimming the skeleton for a checklist sees one fewer line than before, which is why it is recorded here.

### Fixed

- **`zip(..., strict=True)` appears in four places and is a Python 3.10 keyword, so `requires-python = ">=3.9"` was a claim the package could not honour.** On 3.9 it is not a lint warning but a `TypeError` raised at the call site, and one of the four sites is library code: `ascep.bench.workloads` raised it while sizing a multi-image record, meaning a 3.9 user building a vision workload got a stack trace instead of a benchmark. The other three are a test and two figure-generator loops. All four now assert the length equality the keyword was buying and zip without it, so the check survives on every interpreter in the support matrix rather than only the half that never needed it. The CI `tests (py3.9)` job had been failing on this before the 0.6.0 work began; the `lint` job was separately failing `ruff format --check` on `tools/make_figure.py`, introduced by the figure commit, and is reformatted here.

### Migration

A 0.5 `hardware.json` needs exactly one line — `"memory_architecture": "discrete"` — plus `"dense_flops_precision"` naming the precision if it quotes a FLOP/s figure. Thirteen declarations published in this repository were migrated that way (six standalone `hardware.json`, seven `hardware` blocks inside a `report.json`), along with the thirteen documents of the negative corpus, which is regenerated from `examples/negative/baseline.json`. No other field moved and no number changed. Both quoted figures were re-checked against the vendor sheet while doing it, and both were already dense: H100 SXM5 is 989 TFLOP/s dense BF16 — 1,979 is the sparse figure — and GB200 is 2.5 PFLOP/s dense BF16 per GPU. Existing declarations keep the `ascep_version` they were produced under, since this repository does not restamp old reports, so a 0.1.0 declaration validating against the 0.6.0 schema is expected behaviour and not a finding.

**The declarations inside a manifest-pinned reproduction bundle were deliberately not migrated, and that is the more interesting half of this note.** Five published examples carry a `bundle/` whose every artifact is pinned by sha256 in `manifest.json`, and `bundle/declarations/hardware.json` is one of those artifacts: it is a record of what was declared at run time, not a living document. Migrating it would mean rewriting captured bytes and then re-signing the manifest over the rewrite — which is precisely the move the manifest exists to make detectable, and the fact that it would have been done for an innocent reason is what makes the precedent worth refusing. `tools/redact_bundle.py` is the one sanctioned way to change bundle bytes, and it only does so by recording the original digest beside the new one. So those five bundles still contain 0.1.0-shaped declarations that the 0.6.0 schema would reject, and that is correct: the published `report.json` beside each bundle is the document the current schema grades, and the bundle is the evidence it was built from.
