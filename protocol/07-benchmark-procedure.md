...
The reported maximum sustainable concurrency MUST then be re-tested with at least one additional independent repetition at that rung after the search completes. A boundary located by search and never confirmed is a single sample: its surviving repeat is the only evidence the rung passes at all.
...
The censoring cause MUST be named, because the fixes differ. If the ladder stopped because the server reached a gate, the bound is a server result awaiting a longer ladder. If the ladder was capped because the client harness, generator host or request supply could not offer more load, the server's sustainable concurrency was never probed at all: the result MUST be labelled harness-limited, and the required fix — more generator nodes, raised request supply or a higher ladder — MUST be recorded so a reproducing laboratory does not inherit the same invisible ceiling. A harness-limited run MUST NOT enter the Sustainable tier as a boundary figure at any concurrency above the offered load.
...
| Error rate | every request issued or admitted during the window, with non-completion by the declared drain deadline, or abort, counted as failure | requests never offered to the system, client-side generation defects declared before timing, records from an invalidated window |
