...
Alongside it, `report_path` receives a DRAFT capacity report claiming `non-conforming`. That is the weakest claim the schema offers, and it grades the report rather than the hardware: a load generator observes latency and throughput over HTTP and nothing else, so the roofline comparison, the sizing result, the scaling table and the Theoretical and Recommended tiers are left unknown with reasons rather than estimated. `ascep conformance` is the command that MAY raise the claim; a harness that graded its own output would be asserting the one thing it is not in a position to know. It raises it only when asked: `ascep conformance report.json --raise` writes the computed level into the report and replaces the draft paragraph of `conformance_note` with one saying the grade was computed rather than assumed, carrying across whatever the harness appended after it. Without the flag the checker writes nothing, so it stays safe to run on a report you did not author. It MUST NOT lower a claim: a report claiming more than the checks support is reported and left byte-for-byte as its author wrote it, because a tool that quietly rewrote an overstatement into an accurate claim would erase the evidence that anyone overstated.
...
### Which repetition the row publishes, and the dispersion beside it

The row publishes ONE of the rung's counted repetitions, never an average of them, and which one is a rule rather than an accident. The harness ranks the counted windows by `output_tok_s`, breaks ties by `ttft_p95_s`, sorts a window with no measured tail last within its throughput group, and takes the LOWER median — index `(n − 1) // 2` of the sorted windows — so an even count publishes the slower of the two middle windows.

The tie-break is load-bearing rather than defensive. Under `ignore_eos` with a declared output length every completed request emits exactly that many tokens, so two windows that complete the same number of requests tie on `output_tok_s` to every digit — the ordinary case at a saturated rung, not an edge one (§4.2). Ranked on throughput alone, a stable sort then has nothing left to do and submission order decides, so "the median repetition" silently becomes "the second window submitted". On one GB200 multi-image ladder every rung tied on throughput and so published its second window, and at concurrency 32 that meant publishing 8.1633 s from a rung whose three windows measured 8.8817, 8.1633 and 8.6838 s: the fastest window, offered to the reader as typical. A window with no measured tail sorts last within its group for the same reason — an unmeasured tail is not a short one, and floating it to the front displaces a measured window and publishes the faster survivor in its place.

Pooling the repetitions into the row instead is not available. The row's TTFT, ITL, end-to-end latency and throughput all come from the same declared window, so a reader may divide one by another and get a property of the system; a row assembled from per-figure medians would report a combination no window ever exhibited at once, and that arithmetic describes an imaginary machine.

Dispersion is therefore reported beside the row rather than inside it. Where a rung has at least two counted repetitions its row carries a `dispersion` block giving minimum, lower median, maximum, the count of repetitions that produced the figure, and the relative spread, for the four gate-bearing statistics — `ttft_p95_s`, `itl_p95_s`, `e2e_p95_s`, `error_rate_pct` — and for `output_tok_s`. §6's dispersion requirement is met by this block and not by the row. Two things follow that a reader must hold onto. A figure's dispersion median is the median of that figure alone and need not equal the row's value for it, because the row is one window selected once; the two agreeing is common and is not a check. And a rung with fewer than two counted repetitions carries `dispersion: null` beside a `dispersion_u_reason`, never three equal numbers and never a missing key: dispersion over one window is not a narrow spread but no measurement of spread, a published `0.0` would report perfect stability as a finding, and an absent key would make an unmeasured rung indistinguishable from one the harness forgot to fill. **C12** grades that distinction.

The spread is often the figure most likely to change a reader's mind, and it is what makes §5's worst-served-user rule legible on the page. On the multi-image ladder above, per-repetition `ttft_p95_s` varied by 0.8% at concurrency 64 and by 31.5% at concurrency 7 — wider than the effect that campaign had been built to detect, and invisible on a page showing one number per rung. Concurrency 7 is the case worth studying. Its three windows measured 2.2554, 2.4452 and 3.0255 s against a 2.5 s `ttft_p95_max_s` gate, so two passed and one missed by 21%; the row published the lower median, 2.4452 s, and reported `slo_pass: true` beside `outcome: failed`. §5 grades the rung on its worst window and is right to fail it, but a reader holding only the row sees a figure inside the gate and a verdict outside it with nothing between them. The `dispersion` block is that missing sentence, and until it existed the only way to recover the third window was to re-reduce the bundle.
