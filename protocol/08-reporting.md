...
| C5 — binding constraint | every capacity figure names its `Constraint` — `weights`, `kv`, `throughput`, `slo` | §5 table shows users but the constraint column says "n/a" | take the constraint from `capacity_at`'s return value; without it the reader cannot tell whether to buy memory or compute |
...
A violation in C1–C5 alone downgrades the report to **non-conforming**; meeting C1–C5 but failing any of C6–C8 yields **partial**; meeting all eight is **full**.
...
|---|---|---|
| **full** | C1–C8 all pass | comparable with any full or partial conforming report, declared fields permitting |
| **partial** | C1–C5 pass; at least one of C6–C8 fails | MAY be published, MUST be labelled partial, comparability caveated by the failed rule |
