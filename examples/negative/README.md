...
| c12 | C12 | `run.results.2.dispersion` deleted, so the sustainable rung publishes one window of three with no spread beside it | partial |
...
Run `python examples/negative/build_corpus.py` to rebuild all twelve cases from `baseline.json`. `tests/test_negative_corpus.py` fails if a committed report has drifted from what the builder emits, so edit the builder or the baseline -- not the generated case files -- when the corpus needs to change.
